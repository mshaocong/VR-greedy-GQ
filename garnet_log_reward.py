...
def easy_plot3(hist, color, label, cut_off=None, percentile=90, fill=True):
    ...
    if cut_off is None:
        ...
        ax1.plot(new_x[:1+int(np.floor(cut_off/180))], list(avg_loss[:1+int(np.floor(cut_off/180))]), c=color, label=label)

    if fill:
        if cut_off is None:
            raise NotImplementedError
        else:
            ax1.fill_between(new_x[:1+int(np.floor(cut_off/180))], lower_loss[:1+int(np.floor(cut_off/180))], upper_loss[:1+int(np.floor(cut_off/180))], color=color, alpha=0.3)



easy_plot3(np.log(r1), "g", "Policy Gradient", cut_off=1500)
easy_plot(np.log(r2), "orange", "Greedy-GQ", cut_off=1500)
easy_plot2(np.log(r3), "b", "VR-Greedy-GQ: M=3000" , cut_off=1500)
...
ax1.set_ylabel(r"Maximum Log Reward")
